 ...
 * Returns an empty object for a missing/blank/invalid value so an unfiltered
 * request still works.
 */
export function parseFilterWire(raw: string | undefined): FilterWire {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') return parsed as FilterWire
  } catch {
    throw createError({
      statusCode: 400,
...
function isFilter(value: unknown): value is Filter {
  ...
  switch (f.type) {
    ...
    case 'inSet':
      return typeof f.field === 'string' && Array.isArray(f.values)
